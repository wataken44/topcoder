  ...
  string ableToUnlock(vector <string> board)
  {
    ...
    TIMES(y0, h) {
      TIMES(x0, w) {
        FOR(y1, y0 + 1, h) {
          FOR(x1, x0 + 1, w) {
            if(board[y0][x0] == '#' && board[y1][x0] == '#' && board[y0][x1] == '#' && board[y1][x1] == '#') {
              board[y0][x0] = '@'; 
              board[y0][x1] = '@'; 
              board[y1][x0] = '@'; 
              board[y1][x1] = '@';
    ...
    if(count >= 4) {
      ...
      if(sx.size() != 0 && find(sx.begin(), sx.end(), rx[0]) == sx.end()) {
        return "NO";
      }else {
        return "YES";
      }
    }else if(count == 2) {
      if(sx.size() != 0 && find(sx.begin(), sx.end(), rx[0]) == sx.end() && find(sx.begin(), sx.end(), rx[1]) == sx.end()) {
        return "NO";
      }else {
        if(ry[0] == ry[1]) {
          return "YES";
      ...
      if(sx.size() != 0) {
        bool any = false;
        TIMES(i, 3) {
          if(find(sx.begin(), sx.end(), rx[i]) != sx.end()) {
            any = true;
          }
        }
        if(!any) return "NO";
