...
class WaterEqualizer {
 ...
 public:
  ...
  {
    double result = 0;
    int sz = waterVolumes.size();

    double sum = 0;
    TIMES(k, waterVolumes.size()) {
      sum += waterVolumes[k];
    }
    double ave = sum / sz;

    TIMES(k, waterVolumes.size()) {
      if(waterVolumes[k] > ave) {
        result += waterVolumes[k] - ave;
      }
    }
...
Returns:double
Method signature:double minTime(vector <int> waterVolumes)
